...
export function NetflixTop10Row({
  ...
  const visibleItems = items.slice(0, TOP10_COUNT);
  if (visibleItems.length < TOP10_COUNT) return null;

  const posterHeight = Math.round(posterWidth * (3 / 2));
  const numberSize = posterHeight;
              ...
              return (
                ...
                >
                  ...
                  >
                    <span
                      aria-hidden
                      className="pointer-events-none absolute left-0 top-1/2 z-[5] -translate-y-1/2 select-none font-display font-black leading-[0.82] tracking-[-0.06em]"
                      style={{
                        fontSize: numberSize,
                        color: "#060608",
                  ...
                  <p
                    ...
                    style={{ maxWidth: numberPad + posterWidth }}
                  >
                    {streamingPreviewDisplayName(preview)}
                  </p>
                </button>
